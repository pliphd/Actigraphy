function parMagDetection(source, destination, epoch, option)
%PARMAGDETECTION do magnitude analysis in activity files using parallel process
%
%   PARMAGDETECTION(SOURCE, DESTINATION, EPOCH, OPTION) run dfa on all
...
%   OPTION is a struct specifying region and starttime.
%
%   $Author:    Peng Li
%
...
parfor idx = 1:numel(allFiles)
    ...
    runFile('detMag', curFile, destination, epoch, c.Value, option);
...
tbl = vertcat(tblLab{:});
tbl.Properties.VariableNames = {'filename', 'minscale', 'maxscale', 'alpha', 'goodness', 'intercept'};
writetable(tbl, fullfile(destination, 'mag_stat.sum'), 'FileType', 'text');
