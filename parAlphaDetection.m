function parAlphaDetection(source, destination, epoch, option)
%PARALPHADETECTION do dfa in activity files using parallel process
%
%   PARALPHADETECTION(SOURCE, DESTINATION, EPOCH, OPTION) run dfa on all
...
parfor idx = 1:numel(allFiles)
    ...
    runFile('detAlpha', curFile, destination, epoch, c.Value, option);
...
writetable(tbl, fullfile(destination, 'alpha_stat.sum'), 'FileType', 'text');
